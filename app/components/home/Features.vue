<script setup lang="ts">
/**
 * █ [HOME] :: FEATURES
 * =====================================================================
 * DESC:   Features grid section for the home page.
 *         Displays cards with icon, title, description and optional image.
 * STATUS: STABLE
 * =====================================================================
 */



// =============================================================================
// █ CONFIG: CARDS DATA
// =============================================================================
const cards = ref([
    {
        title: 'Tema',
        description: 'Aprende a personalizar los componentes de Nuxt UI usando Tailwind CSS v4.',
        icon: 'i-lucide-swatch-book',
        class: 'lg:col-span-2',
        image: {
            path: 'https://ui2.nuxt.com/illustrations/color-palette',
            width: 363,
            height: 152
        },
        orientation: 'horizontal' as const
    },
    {
        title: 'Fuentes',
        description: 'Nuxt UI se integra con Nuxt Fonts para proporcionar optimización de fuentes plug-and-play.',
        icon: 'i-lucide-a-large-small',
        variant: 'soft' as const
    },
    {
        title: 'Modo de Color',
        description: 'Nuxt UI se integra con Nuxt Color Mode para alternar entre claro y oscuro.',
        icon: 'i-lucide-sun-moon',
        variant: 'soft' as const
    },
    {
        title: 'Iconos',
        description: 'Nuxt UI se integra con Nuxt Icon para acceder a más de 200,000 iconos de Iconify.',
        icon: 'i-lucide-smile',
        image: {
            path: 'https://ui2.nuxt.com/illustrations/icon-library',
            width: 362,
            height: 184
        },
        class: 'lg:col-span-2',
        orientation: 'horizontal' as const,
        reverse: true
    }
])
</script>

<template>
    <!-- ======================================================================= -->
    <!-- █ COMPONENT: FEATURES -->
    <!-- ======================================================================= -->
    <UPageSection title="Características" description="Explora las características principales de Nuxt UI"
        headline="Características" />
    <UPageGrid>
        <UPageCard v-for="(card, index) in cards" :key="index" v-bind="card">
            <UColorModeImage v-if="card.image" :light="`${card.image.path}-light.svg`"
                :dark="`${card.image.path}-dark.svg`" :width="card.image.width" :height="card.image.height"
                :alt="card.title" loading="lazy" class="w-full" />
        </UPageCard>
    </UPageGrid>
</template>
